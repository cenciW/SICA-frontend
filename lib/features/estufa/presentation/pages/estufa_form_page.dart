...
class _EstufaFormPageState extends State<EstufaFormPage> {
  ...
  void initState() {
    ...
    _localizacaoController = TextEditingController(text: widget.estufa?.localizacao ?? '');
    _larguraController = TextEditingController(text: widget.estufa?.largura.toString() ?? '');
    _alturaController = TextEditingController(text: widget.estufa?.altura.toString() ?? '');
    _comprimentoController = TextEditingController(text: widget.estufa?.comprimento.toString() ?? '');
    _volumeTotalController = TextEditingController(text: widget.estufa?.volumeTotal?.toString() ?? '');
    _firmwareVersaoController = TextEditingController(text: widget.estufa?.firmwareVersao ?? '');
          ...
          child: ListView(
            children: [
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(labelText: 'Versão do Firmware'),
              ),
                ...
                onPressed: () async {
                  if (_formKey.currentState!.validate()) {
                    final data = {
                      ...
                      'volume_total': double.tryParse(_volumeTotalController.text),
                      'status': _status,
                    ...
                    if (isEditing && token != null) {
                      success = await provider.updateEstufa(widget.estufa!.id, data, token);
                    } else {
                      success = await provider.createEstufa(data);
                    }
